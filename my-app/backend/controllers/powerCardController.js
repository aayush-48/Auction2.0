...
export const createPowerCard = async (req, res) => {
  try {
    ...
    res.status(400).json({ message: "Invalid power card data", error: error.message })
  }
}

export const updatePowerCard = async (req, res) => {
  try {
    const powerCard = await PowerCard.findByIdAndUpdate(req.params.id, req.body, { new: true })
    if (powerCard) {
      res.json(powerCard)
    } else {
