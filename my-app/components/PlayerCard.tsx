...
interface PlayerCardProps {
  ...
  rtmTeam?: "CSK" | "DC" | "GT" | "KKR" | "LSG" | "MI" | "PBKS" | "RCB" | "RR" | "SRH"
  isElite?: boolean
  src?: string
}
...
const PlayerCard: React.FC<PlayerCardProps> = ({
  ...
  const calculateOverallRating = () => {
    ...
    if (type.toLowerCase() === "batsman" || type.toLowerCase() === "wicket-keeper") {
      const battingAvg = (batting.powerplay + batting.middleOvers + batting.deathOvers) / 3
      overallRating = (battingAvg + rtmElite + captaincy) / 3
    } else if (type.toLowerCase() === "bowler") {
      const bowlingAvg = (bowling.powerplay + bowling.middleOvers + bowling.deathOvers) / 3
  ...
  const showBatting =
    type.toLowerCase() === "batsman" || type.toLowerCase() === "wicket-keeper" || type.toLowerCase() === "all-rounder"
  ...
  return (
    ...
    >
      <div className="flex items-center mb-4">
        <Image src={src || "/placeholder.svg"} alt={name} width={64} height={64} className="rounded-full mr-4" />
        <div>
          <h3 className="text-xl font-bold text-heliotrope">{name}</h3>
          <p className="text-sm text-mauve">{`${country} | ${gender} | ${type}`}</p>
          <p className="text-lg text-heliotrope">Overall Rating: {calculateOverallRating()}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div>
          ...
          <p className="text-xl font-bold text-white">{formatPrice(basePrice)}</p>
        </div>
        <div>
          <p className="text-sm text-mauve">Final Price</p>
          <p className="text-xl font-bold text-heliotrope">{formatPrice(finalPrice)}</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex justify-between items-center pt-4 border-t border-french-violet">
          <div>
            ...
            <p className="text-lg font-bold text-heliotrope">{rtmTeam || "No RTM"}</p>
          </div>
          <div>
            <h4 className="text-sm text-mauve mb-1">Captaincy</h4>
            <p className="text-lg font-bold text-heliotrope">{ratings.captaincy}</p>
          </div>
          {isElite && (
            <div>
