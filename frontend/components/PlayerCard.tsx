...
const RatingBar = ({ rating, label }: { rating: number; label: string }) => (
  <div className="flex items-center mb-2 group">
    <span className="w-24 text-sm text-amber-200 group-hover:text-amber-400 transition-colors">{label}</span>
    ...
    <span className="ml-2 text-sm text-amber-200 group-hover:text-amber-400 transition-colors">{rating}</span>
  </div>
);

const RatingCircle = ({ rating, label, highlight = false }: { rating: number; label: string; highlight?: boolean }) => (
  <div className={`flex flex-col items-center ${highlight ? 'scale-110' : ''}`}>
    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-cyan-200 font-bold
      ${highlight 
      ...
      >
        ...
        >
          <div className="w-full h-full p-4 flex flex-col bg-gradient-to-br from-blue-900/40 to-transparent relative">
            <h3 className="text-lg font-bold text-white mb-4">Player Ratings</h3>
           
            {showBatting && (
              <div className="mb-4">
                <h4 className="text-sm font-semibold text-cyan-200 mb-2 flex items-center">
                  <GiCricketBat className="mr-2 text-cyan-400 text-lg" /> Batting
                </h4>
                <div className="space-y-2">
                  <RatingBar rating={ratings.batting.powerplay} label="Powerplay" />
                  <RatingBar rating={ratings.batting.middleOvers} label="Middle" />
                  <RatingBar rating={ratings.batting.deathOvers} label="Death" />
                </div>
              </div>
            )}
            ...
            {showBowling && (
              <div className="mb-4">
                <h4 className="text-sm font-semibold text-cyan-200 mb-2 flex items-center">
                  <FaBowlingBall className="mr-2 text-cyan-400 text-lg" /> Bowling
                </h4>
                <div className="space-y-2">
                  <RatingBar rating={ratings.bowling.powerplay} label="Powerplay" />
                  <RatingBar rating={ratings.bowling.middleOvers} label="Middle" />
                  <RatingBar rating={ratings.bowling.deathOvers} label="Death" />
                </div>
              </div>
            )}
          </div>
